...
from llm_wiki.infrastructure.persistence.postgres.mappers import EntityMapper, EventEntityLinkMapper, EntityRelationMapper
...
class PostgresEntityRepository(EntityRepository):
    ...
    async def search_relations(
        ...
        q = select(orm.EntityRelation).where(
            orm.EntityRelation.from_entity_id == entity_id.value
        )
        if relation_type:
