...
VALID_PREDICATES: set[str] = {
    ...
    "is_subsidiary_of", "owns", "acquired_by", "merged_with", "spin_off_from",
    "partner_of", "customer_of", "creditor_of", "licenses_to",
    # 1b. Geographic (pre-existing high-frequency predicates)
    "located_in", "headquartered_in",
    # 2. Leadership
    "led_by", "founded_by", "works_for", "major_shareholder",
    # 3. Market & Competition
    "competes_with", "supplies_to", "distributes", "disrupts",
    # 4. Sector
    "belongs_to_sector", "sector_leader", "sector_benefits_from",
    "sector_hurt_by", "sector_impacted_by", "sector_weight_in_index",
    # 5. Investment
    "invested_in", "shareholder_of", "funded_by",
    # 6. Bonds
    "yield_inverse_to", "competes_for_capital_with", "issued_by",
    "spread_over", "rated_by", "rating_impact", "affected_by_exchange_rate",
    # 7. Gold
    "inverse_to", "priced_at_premium_to", "hedge_against",
    "supply_controlled_by", "safe_haven_when",
    # 8. Crypto
    "correlates_with_risk_on", "regulated_by", "banned_in",
    "leads", "dominance_over", "mining_dependent_on", "pegged_to",
    # 9. Real Estate
    "develops", "benefits_from_infrastructure", "zoning_affects",
    "infrastructure_drives_price", "tax_policy_affects",
    "interest_rate_sensitivity", "credit_growth_dependent",
    "has_price_per_sqm", "has_price_growth", "has_rental_yield",
    # 10. Macro
    "correlated_with", "inversely_correlated", "leads", "lags",
    "tightens", "stimulates", "targets", "drives_price_of",
    # 11. Cross-Border
    "spillover_impacts", "capital_flows_from", "export_competitor_of",
    "trade_surplus_with", "trade_deficit_with", "largest_import_from",
    "largest_export_to", "depreciates", "triggers_inflation_in",
    "reallocates_supply_chain_to",
    # 12. Financial Metrics
    "has_market_cap", "has_pe_ratio", "has_revenue", "has_profit",
    "has_dividend_yield", "has_roe", "has_foreign_ownership",
    "has_growth_rate", "has_debt_to_equity", "has_npl_ratio",
    "constituent_of", "has_weight_in",
}

# ── Direction rules: which (from_type, to_type) pairs are FORBIDDEN ─────────

COMPANY_TYPES: frozenset[str] = frozenset({
    "stock_ticker", "company", "bank", "securities_firm", "fund",
    "real_estate_developer",
})
PERSON_TYPES: frozenset[str] = frozenset({
    "person", "executive", "founder", "analyst", "investor",
})
...
PREDICATE_TYPE_MATRIX: dict[str, list[tuple[str, str]]] = {
    # 1. Corporate Structure
    "is_subsidiary_of": [("stock_ticker", "stock_ticker"), ("company", "company"), ("bank", "bank")],
    "owns": [("stock_ticker", "stock_ticker"), ("company", "company"), ("bank", "bank"), ("stock_ticker", "real_estate_project")],
    ...
    "headquartered_in": [("stock_ticker", "location"), ("company", "country"), ("company", "city"), ("bank", "country")],
    ...
    "works_for": [("executive", "stock_ticker"), ("person", "stock_ticker"), ("analyst", "securities_firm")],
    "major_shareholder": [("person", "stock_ticker"), ("investor", "stock_ticker"), ("fund", "stock_ticker")],

    # 3. Market & Competition
    "competes_with": [("stock_ticker", "stock_ticker"), ("company", "company"), ("bank", "bank")],
    "supplies_to": [("stock_ticker", "stock_ticker"), ("company", "company"), ("stock_ticker", "real_estate_developer")],
    ...
    "sector_benefits_from": [("sector", "macro_indicator"), ("sector", "interest_rate"), ("sector", "policy")],
    "sector_hurt_by": [("sector", "interest_rate"), ("sector", "policy"), ("sector", "exchange_rate")],
    "sector_impacted_by": [("sector", "policy"), ("sector", "trade_policy"), ("sector", "monetary_policy")],
    "sector_weight_in_index": [("sector", "market_index")],

    # 5. Investment
    "invested_in": [("stock_ticker", "stock_ticker"), ("fund", "stock_ticker"), ("stock_ticker", "real_estate_project"), ("bank", "stock_ticker")],
    "shareholder_of": [("stock_ticker", "stock_ticker"), ("fund", "stock_ticker"), ("investor", "stock_ticker")],
    ...
    "competes_for_capital_with": [("bond", "market_index"), ("bond", "sector"), ("precious_metal", "market_index"), ("real_estate_project", "market_index")],
    ...
    "benefits_from_infrastructure": [("real_estate_project", "infrastructure_project"), ("location", "infrastructure_project")],
    "zoning_affects": [("policy", "real_estate_project"), ("policy", "location")],
    "infrastructure_drives_price": [("infrastructure_project", "real_estate_project"), ("infrastructure_project", "location")],
    "tax_policy_affects": [("policy", "real_estate_developer"), ("tax_policy", "real_estate_project")],
    "interest_rate_sensitivity": [("real_estate_developer", "interest_rate"), ("sector", "interest_rate")],
    "credit_growth_dependent": [("real_estate_developer", "macro_indicator"), ("sector", "macro_indicator")],
    "has_price_per_sqm": [("location", "financial_metric"), ("real_estate_project", "financial_metric")],
    "has_price_growth": [("location", "financial_metric"), ("real_estate_project", "financial_metric")],
    "has_rental_yield": [("location", "financial_metric"), ("real_estate_project", "financial_metric")],
    ...
    "stimulates": [("policy", "macro_indicator"), ("fiscal_policy", "macro_indicator"), ("monetary_policy", "sector")],
    "targets": [("policy", "macro_indicator"), ("monetary_policy", "inflation")],
    "drives_price_of": [("macro_indicator", "commodity"), ("exchange_rate", "precious_metal"), ("interest_rate", "precious_metal")],

    # 11. Cross-Border
    "spillover_impacts": [("country", "market_index"), ("policy", "market_index"), ("interest_rate", "exchange_rate")],
    ...
    "triggers_inflation_in": [("commodity", "country"), ("energy", "country"), ("exchange_rate", "country")],
    "reallocates_supply_chain_to": [("country", "industrial_park"), ("country", "economic_zone"), ("country", "country")],
    ...
    "has_growth_rate": [("stock_ticker", "financial_metric"), ("company", "financial_metric"), ("macro_indicator", "financial_metric")],
...
def validate_relation(
    ...
    if _is_direction_forbidden(from_type, to_type):
        return ValidationResult(False, f"direction forbidden: {from_type}→{to_type} (company→person or person→person)")
    ...
    if allowed is not None and len(allowed) > 0 and (from_type, to_type) not in allowed:
        ...
        return ValidationResult(False,
            f"type mismatch: {predicate} not valid for {from_type}→{to_type}")
...
def validate_relations_batch(
    ...
    for rel in relations:
        ...
        if result.valid:
            ...
            rejected += 1
            reasons.append(f"{rel.get('from','?')} —{rel.get('predicate','?')}→ {rel.get('to','?')}: {result.reason}")
